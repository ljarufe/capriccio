...
#from projectcapriccio.ventas.models import Producto_item, Personalizado_item
...
class NoticeType(models.Model):
    ...
    class Meta:
        verbose_name = _("Tipo de Notificacion")
        verbose_name_plural = _("Tipos de Notificacion")
...
class NoticeSetting(models.Model):
    ...
    class Meta:
        verbose_name = _("Configuracion de Notificacion")
        verbose_name_plural = _("Configuracion de Notificaciones")
...
class Compra(models.Model):
    """
    Objeto creado con una compra del carrito
    Factura: cabecera
    """
    ...
    direccion_entrega = models.ForeignKey(Direccion,
    ...
    def __unicode__(self):
        return u'%s para %s' % (self.cliente, self.fecha_entrega)
    ...
    def save(self):
        ...
        #client = self.cliente
        if self.cliente:
            usuarioCompra = self.cliente.user
            lugar = str(self.direccion_entrega.ciudad).lower()
            estados = {'aceptado':'A','pendiente':'P','nulo':'N'}
            if self.status is not "null":
                if self.status == estados["aceptado"]:
                    if self.abono == False:
                        for i in usuario:
                            if i.is_staff == 1:
                                city = i.groups.all()
                                try:
                                    ciudad = str(city[0]).lower()
                                    if lugar ==  ciudad:
                                        a = send_now([i],"Pedido",self,"Nueva compra")
                                ...
                                pass
                        sending = send_now([usuarioCompra], "compra", self,"Nueva Compra")
            return "Correcto"
        else:
            pass
        
    def ciudad(self):
        if self.direccion_entrega.ciudad:
             return u'<label style="background-color:#fff;color:red;">%s</label>' % self.direccion_entrega.ciudad
    
    ciudad.short_description = "Ciudad"
    ciudad.allow_tags = True
    ...
    def precio_cambio(self):
        ...
        if self.total:
            ...
            cambio = round(self.total/tipo_cambio, 2)          
        return cambio
    precio_cambio.short_description = "USD"
            
    def filtro_ciudad(self):
        if self.direccion_entrega:
            return self.direccion_entrega.ciudad
    filtro_ciudad.short_description = "filtro"    
...
class Notice(models.Model):     
    ...
    venta = models.ForeignKey(Compra)
...
def send_now(users, label, buy, extra_context=None, on_site=True):
    """
    ...
    try:
        from projectcapriccio.ventas.models import *
    	producto = Producto_item.objects.filter(compra = buy)
    	personalizado = Personalizado_item.objects.get(id=buy.id)
    except:
        #producto = "no existe"
        #personalizado = "no existe"
	...
	if user.is_superuser == 1:
	        notice = Notice.objects.create(user=user, message=messages['mensaje.html'],
		            notice_type=notice_type, on_site=on_site,venta=buy)
	else:
